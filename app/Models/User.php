...
class User extends WmUser
{
    ...
    public function canManageHikingRoute(HikingRoute $hr): bool
    {
        ...
        if (in_array(UserRole::Administrator, $roles) || in_array(UserRole::NationalReferent, $roles)) {
            return true;
        }

        if (in_array(UserRole::RegionalReferent, $roles)) {
            return $hr->regions->pluck('id')->contains($this->region->id);
        }

        if (in_array(UserRole::LocalReferent, $roles)) {
